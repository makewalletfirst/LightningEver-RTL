...
export class ECLLightningPaymentsComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  sendPayment() {
    ...
    if (!this.paymentDecoded.amount || this.paymentDecoded.amount === 0) {
      ...
      const titleMsg = 'It is a zero amount invoice. Enter the amount (ever) to pay.';
      this.store.dispatch(openConfirmation({
        payload: {
          data: {
            ...
            getInputs: [
              { placeholder: 'Amount (ever)', inputType: DataTypeEnum.NUMBER, inputValue: '', width: 30 }
            ]
          }
        }
      }));
      ...
      const reorderedPaymentDecoded: MessageDataField[][] = [
        ...
        { key: 'amount', value: this.paymentDecoded.amount, title: 'Amount (ever)', width: 50, type: DataTypeEnum.NUMBER }],
              ...
              this.commonService.convertCurrency(+this.paymentDecoded.amount, CurrencyUnitEnum.SATS, CurrencyUnitEnum.OTHER, (this.selNode.settings.currencyUnits && this.selNode.settings.currencyUnits.length > 2 ? this.selNode.settings.currencyUnits[2] : ''), this.selNode.settings.fiatConversion).
                ...
                subscribe({
                  next: (data) => {
                    ...
                    this.paymentDecodedHintPre = 'Sending: ' + this.decimalPipe.transform(this.paymentDecoded.amount ? this.paymentDecoded.amount : 0) + ' ever (';
                    ...
                    this.paymentDecodedHintPre = 'Sending: ' + this.decimalPipe.transform(this.paymentDecoded.amount ? this.paymentDecoded.amount : 0) + ' ever | Memo: ' + this.paymentDecoded.description + '. Unable to convert currency.';
                    this.paymentDecodedHintPost = '';
                  }
                });
            } else {
              this.paymentDecodedHintPre = 'Sending: ' + this.decimalPipe.transform(this.paymentDecoded.amount ? this.paymentDecoded.amount : 0) + ' ever | Memo: ' + this.paymentDecoded.description;
              this.paymentDecodedHintPost = '';
            }
          } else {
  ...
  showPartView(selPart: PaymentSentPart, selPayment: PaymentSent, sentPaymentInfo?: any[]) {
    const reorderedPart: MessageDataField[][] = [
      ...
      [{ key: 'amount', value: selPart.amount, title: 'Amount (ever)', width: 50, type: DataTypeEnum.NUMBER },
      { key: 'feesPaid', value: selPart.feesPaid, title: 'Fee (ever)', width: 50, type: DataTypeEnum.NUMBER }]
    ];
