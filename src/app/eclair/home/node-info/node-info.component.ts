...
export class ECLNodeInfoComponent implements OnChanges {
  ...
  ngOnChanges() {
    this.chains = [];
    // LightningEver: replace 'Bitcoin' with 'BitEver' on the dashboard Chain card
    this.chains.push('BitEver ' + (this.information.network ? this.commonService.titleCase(this.information.network) : 'Testnet'));
  }
